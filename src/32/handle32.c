...
int handle32_symtab(Elf32_Shdr *section_h, Elf32_Ehdr *elf_header, char *file_data, int n, int file_size)
{
    ...
    sh_offset = read_uint32(section_h[n].sh_offset, file_data);

    symtab = (Elf32_Sym *)(file_data + sh_offset);                                                                // OK
    symtab_size = read_uint32(section_h[n].sh_size, file_data) / read_uint32(section_h[n].sh_entsize, file_data); // OK
...
int handle32(char *file_data, Elf32_Ehdr *elf_header, struct stat fd_info)
{
    ...
    {
        if (read_uint32(section_h[i].sh_name, file_data) > read_uint32(section_h[e_shstrndx].sh_size, file_data))
            return fprintf(stderr, "Bad section table header\n");

        sh_type = read_uint32(section_h[i].sh_type, file_data);
        if (sh_type == SHT_SYMTAB)
            return handle32_symtab(section_h, elf_header, file_data, i, fd_info.st_size);
    }
    return fprintf(stderr, "Symbol table or string table not found\n");
}
